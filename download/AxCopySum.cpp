...
const int iVersionMinor = 93;
const wchar_t * gszSimpleSummerName =_T("AxSum");
const wchar_t * gszSummerName = _T("AxSumCopy");

// Forward
namespace my {
    int tcprintf(FILE *fp, const wchar_t *szFmt, ...);
}
...
void
AssFunc(bool fOk, const wchar_t *sz, const wchar_t *szFile, int iLine) {
    if (!fOk) {
...
/// \param sz The message to display.
void
ChkFunc(bool fOk, const wchar_t *sz) {
    if (!fOk) {
...
void
ChkApiFunc(bool fOk, const wchar_t *szLastError, const wchar_t *szMsg = _T("")) {
    if (!fOk) {
        if (szMsg && *szMsg) {
...
int
tcprintf(FILE *fp, const wchar_t *szFmt, ...) {
    va_list vaArgs;
    ...
    HANDLE hDest = (HANDLE)_get_osfhandle(_fileno(fp));
...
GetHexString(const unsigned char* bytes, size_t cb) {
    wchar_t *sHex = new wchar_t[cb + cb + 1];
    const wchar_t sHexDigits[] = _T("0123456789abcdef");
...
private:
    ...
    const wchar_t *m_szFileIn;               ///< The expanded full input file name
    const wchar_t *m_szFileOut;              ///< The expanded full input file name
...
public:
    ...
    int CopyOrVerify(const wchar_t *szFileIn, const wchar_t *szFileOut) {
        int iReturn;
        try {
...
private:
    ...
    size_t BufferLen(int index) {
        ULARGE_INTEGER offsetThis;
        offsetThis.LowPart = m_pOverlapped[index].Offset;
        offsetThis.HighPart = m_pOverlapped[index].OffsetHigh;
        if (offsetThis.QuadPart + m_cbBufferSize > (ULONGLONG)m_offsetFileSize.QuadPart) {
    ...
    ReadNextBuffer(DWORD dwErrorCode, DWORD dwNumberOfBytesTransfered, LPOVERLAPPED pOverlapped) {
        ...
        if (m_offsetNextRead.QuadPart + cbBytesToRead > (ULONGLONG)m_offsetFileSize.QuadPart) {
    ...
    /// \param szFileOut The output file, or NULL if no copy just summing
    /// \return The status code to return. Zero for ok.
    int Process(const wchar_t *szFileIn, const wchar_t *szFileOut) {
        ...
        if (m_fVerify) {
            ...
            if (m_iVerbosityLevel >= 3) {
                CheckNewLineNeed();
                my::tcprintf(stderr, _T("MD5 of file read is %s\n"), auto_arr<wchar_t>(GetHexString(m_hash, sizeof m_hash)));
            }
        }
    ...
    DWORD GetDiskInfo(const wchar_t *szFilePath, bool *pfIsNetwork) {
        wchar_t szWork[MAX_PATH];
    ...
    /// \param szFileIn The input file to read from
    /// \param szFileOut The output file, or NULL if no copy just summing
    void SetFiles(const wchar_t *szFileIn, const wchar_t *szFileOut) {
        m_szFileIn = szFileIn;
...
public:
    CSimpleSummerMode() {
        ...
        fChecksumOnly = false;              ///< Set default checksum only option
    }

    int Main(int argc, wchar_t* argv[], wchar_t *szProgramName) {
        if (argc == 1) {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (fSingleFile) {
                my::tcprintf(stderr, _T("%s\n"), auto_arr<wchar_t>(GetHexString(theSummer.GetHash(), theSummer.GetHashLen())));
            } else {
                my::tcprintf(stderr, _T("%s %s\n"), auto_arr<wchar_t>(GetHexString(theSummer.GetHash(), theSummer.GetHashLen())), argv[i]);
            }
        }
    ...
    void Usage(wchar_t *sName, int nBuffers, size_t cbBufferSize, bool fVerify, bool fChecksumOnly) {
        Copyright(sName);
        my::tcprintf(stderr,
            _T("Usage: %s InFile[s]\n\n")
...
extern int optind;
int getopt(int argc, wchar_t *argv[], const wchar_t *optstring);
}
...
public:
    CFullMode() {
        ...
        m_fChecksumOnly = false;              ///< Set default checksum only option
        m_fDisplayChecksum = false;           ///< Set default checksum display option
    }

    void ParseOptions(int argc, wchar_t *argv[], wchar_t*szProgramName) {
        ...
        int c;
        while ((c = my::getopt(argc, argv, (wchar_t *)L"vCcVqn:s:S:")) != EOF) {
            switch (c) {
                ...
                case _T('s'):
                case _T('S'):
                    m_cbBufferSize = (size_t)_wtol(my::optarg) * 1024 * (c == L's' ? 1 : 1024);
    ...
    int Main(int argc, wchar_t *argv[], wchar_t *szProgramName) {
        ...
        if ((m_fDisplayChecksum && m_iVerbosityLevel >= 1) || m_fChecksumOnly) {
            my::tcprintf(stderr, _T("%s%s\n"), m_iVerbosityLevel > 0 ? _T("MD5 is ") : _T(""), auto_arr<wchar_t>(GetHexString(theCopier.GetHash(), theCopier.GetHashLen())));
        }
        ...
        if (m_fVerify && !m_fChecksumOnly) {
            ...
            if (m_iVerbosityLevel >= 1) {
                ...
                if (m_fDisplayChecksum) {
                    ...
                    my::tcprintf(stderr, _T("MD5 is %s\nVerify succeeded\n"), auto_arr<wchar_t>(GetHexString(theCopier.GetHash(), theCopier.GetHashLen())));
                }
            }
        }
        ...
        if (m_iVerbosityLevel >= 1) {
            ...
            if (timeMilliSeconds / 1000) {
                ...
                const wchar_t * szSizeNames[] = {_T("KB"), _T("MB"), _T("GB"), _T("TB") ,_T("PB") };
...
int _tmain(int argc, wchar_t* argv[])
{
    ...
    wchar_t *szProgramName = PathFindFileName(szModuleName);
